...
def load_monthly_data(file_path):
    ...
    print(
        f"Φορτώθηκαν (Loaded) {len(df)} μήνες δεδομένων (months of data) από (from) {file_path}"
    )
    return df
...
def create_lagged_features(df, n_lags, target_col="Close"):
    ...
    print(
        f"  Δημιουργήθηκαν (Created) {n_lags} χαρακτηριστικά υστέρησης (lagged features) για (for) Close και (and) Volume"
    )
