...
def apply_pca(X_train, X_val, variance_threshold=0.95):
    ...
    Παράμετροι (Args):
        X_train: Χαρακτηριστικά εκπαίδευσης (Training features (already scaled))
        X_val: Χαρακτηριστικά επικύρωσης (Validation features (already scaled))
        variance_threshold: Ελάχιστη αθροιστική διακύμανση να διατηρηθεί (Minimum cumulative variance to retain) (προεπιλογή: 95% / default: 95%)
    ...
    Ανάλυση Κύριων Συνιστωσών (PCA) (Principal Component Analysis (PCA)):
        ...
        4. Οι PCs είναι ορθογώνιες (μη-συσχετιζόμενες) μεταξύ τους
           PCs are orthogonal (uncorrelated) to each other
                ...
                - PCs είναι ιδιοδιανύσματα, επεξηγούμενη διακύμανση είναι ιδιοτιμές (PCs are eigenvectors, variance explained is eigenvalues)
    ...
    Όριο Διακύμανσης (Variance Threshold):
        - 0.95 (95%): Διατηρεί περισσότερες πληροφορίες, μέτρια μείωση (Retains most information, moderate reduction)
...
def compute_cfs_merit(X, y, feature_indices):
    ...
    Τύπος Αξίας CFS (CFS Merit Formula):
        Merit(S) = (k · r̄cf) / √(k + k(k-1) · r̄ff)
          ...
          - r̄cf: Μέση συσχέτιση μεταξύ χαρακτηριστικών και στόχου (κλάσης) (Average correlation between features and target (class))
          - r̄ff: Μέση συσχέτιση μεταξύ χαρακτηριστικών (χαρακτηριστικό-χαρακτηριστικό) (Average correlation between features (feature-feature))

    Ερμηνεία (Interpretation):
                - Αριθμητής: Ανταμείβει υψηλή συσχέτιση χαρακτηριστικών-στόχου (Numerator: Rewards high feature-target correlation)
                - Παρονομαστής: Ποινή σε υψηλή συσχέτιση χαρακτηριστικών-χαρακτηριστικών (πλεονασμός) (Denominator: Penalizes high feature-feature correlation (redundancy))
                - Υψηλότερη αξία = καλύτερο υποσύνολο χαρακτηριστικών (Higher merit = better feature subset)
                - Ισορροπεί σχετικότητα (προς στόχο) με πλεονασμό (μεταξύ χαρακτηριστικών) (Balances relevance (to target) with redundancy (among features))

    Γιατί το CFS είναι Αποτελεσματικό (Why CFS is Effective):
                - Επιλέγει χαρακτηριστικά υψηλής συσχέτισης με στόχο (Selects features highly correlated with target)
                - Αποφεύγει πλεονάζοντα χαρακτηριστικά (π.χ. close_t-1 και close_t-2 είναι όμοια) (Avoids redundant features (e.g., close_t-1 and close_t-2 are similar))
                - Γρήγορος υπολογισμός (βασισμένος σε συσχέτιση, όχι εκπαίδευση μοντέλου) (Fast computation (correlation-based, no model training))
                - Λειτουργεί καλά για γραμμικές σχέσεις (Works well for linear relationships)

    Για Πρόβλεψη Μετοχών (For Stock Prediction):
                - Πρόσφατες υστερήσεις (close_t-1, close_t-2) υψηλής συσχέτισης με στόχο (Recent lags (close_t-1, close_t-2) highly correlated with target)
                - Αυτές οι υστερήσεις επίσης υψηλής συσχέτισης μεταξύ τους (These lags also highly correlated with each other)
                - CFS μπορεί να επιλέξει close_t-1 και να παραλείψει close_t-2 (πλεονάζον) (CFS might select close_t-1 and skip close_t-2 (redundant))
                - Χαρακτηριστικά όγκου λιγότερο συσχετισμένα → μπορεί να συμπεριληφθούν εάν προσθέτουν νέες πληροφορίες (Volume features less correlated → may be included if adding new info)
    """
...
def apply_cfs(X_train, y_train, X_val, feature_names, max_features=None):
    ...
    Παράμετροι (Args):
        ...
        max_features: Μέγιστα χαρακτηριστικά προς επιλογή (None = αυτόματο) (Maximum features to select (None = automatic))
    ...
    Αλγόριθμος CFS (Προς-τα-Εμπρός Επιλογή) (CFS Algorithm (Forward Selection)):
          ...
          3. Προσθήκη χαρακτηριστικού που αυξάνει περισσότερο την αξία (Add feature that most increases merit)
    ...
    Πολυπλοκότητα Χρόνου (Time Complexity):
        ...
        - Πολύ πιο γρήγορο από wrapper μεθόδους (όχι εκπαίδευση μοντέλου) (Much faster than wrapper methods (no model training))
    """
...
def apply_forward_selection(
    ...
    Διαδοχική Προς-τα-Εμπρός Επιλογή (Wrapper Μέθοδος) (Sequential Forward Selection (Wrapper Method)):
        ...
        Αλγόριθμος (Algorithm):
          ...
          2. Για κάθε υπόλοιπο χαρακτηριστικό (For each remaining feature):
              - Εκπαίδευση μοντέλου με τρέχοντα + αυτό το χαρακτηριστικό (Train model with current features + this feature)
              - Αξιολόγηση απόδοσης (cross-validation) (Evaluate performance (cross-validation))
          3. Προσθήκη χαρακτηριστικού που δίνει καλύτερη απόδοση (Add feature that gives best performance)
          4. Επανάληψη μέχρι επιλογή επιθυμητού αριθμού χαρακτηριστικών (Repeat until desired number of features selected)

        Αξιολόγηση (Evaluation):
                - Χρησιμοποιεί πραγματική απόδοση μοντέλου (όχι συσχέτιση) (Uses actual model performance (not correlation))
        ...
        Πλεονεκτήματα (Benefits):
                ...
                - Βελτιστοποιεί για συγκεκριμένο μοντέλο (LinearRegression) (Optimizes for specific model (LinearRegression))
                - Συνήθως δίνει καλύτερη προβλεπτική απόδοση (Usually gives best predictive performance)

        Μειονεκτήματα (Drawbacks):
                - Υπολογιστικά επιβαρυντικό (εκπαιδεύει πολλά μοντέλα) (Computationally expensive (trains many models))
                ...
                - Άπληστος αλγόριθμος (μπορεί να μη βρει καθολικό βέλτιστο) (Greedy algorithm (may not find global optimum))

    Σύγκριση με CFS (Comparison with CFS):
          - CFS: Γρήγορο, βασισμένο σε συσχέτιση, όχι εκπαίδευση μοντέλου (Fast, correlation-based, no model training)
          - Wrapper: Αργό, βασισμένο σε μοντέλο, λαμβάνει υπόψη αλληλεπιδράσεις (Slow, model-based, considers interactions)
          - CFS καλό για γρήγορη επιλογή (CFS good for quick feature selection)
          - Wrapper καλύτερο για τελική βελτιστοποίηση μοντέλου (Wrapper better for final model optimization)
...
def main():
    ...
    2. Εφαρμογή PCA (95% διακύμανση), CFS, και Διαδοχική Προς-τα-Εμπρός Επιλογή σε κάθε ρύθμιση (Apply PCA (95% variance), CFS, and Sequential Forward Selection to each)
    3. Σύγκριση αποτελεσμάτων σε όλες τις μεθόδους και ρυθμίσεις (Compare results across all methods and configurations)
    """
    print("=" * 80)
